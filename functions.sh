...
jv_pg_gm_check_traffic()
{
  if [[ -n "$var_jv_pg_gm_api_key" ]]; then
    local json=$(curl -s -G "https://maps.googleapis.com/maps/api/directions/json" --data "language=$3" --data-urlencode "origin=$1" --data-urlencode "destination=$2" --data "traffic_model=best_guess" --data "departure_time=now" --data-urlencode "key=$var_jv_pg_gm_api_key")
